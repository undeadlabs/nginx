...
describe 'ul-nginx::source' do
  ...
  it 'includes all the source modules recipes' do
    expect(chef_run).to include_recipe('ul-nginx::http_gzip_static_module')
    expect(chef_run).to include_recipe('ul-nginx::http_ssl_module')
  end
